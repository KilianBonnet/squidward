...
class MonteCarlo {
    ...
    runSearch(state, timeout = 95) {
        ...
        while (Date.now() < end) {
            ...
            let theWinner = winnerFinder(node.state.board)

            if (node.isLeaf() === false && theWinner === null) {
                node = this.expand(node);
                theWinner = this.simulate(node);
            }
            this.backpropagate(node, theWinner)

            if (theWinner === 0) draws++
            totalSims++
        }
    ...
    simulate(node) {
        ...
        let winner = winnerFinder(state.board);

        while (winner == null) {
            let plays = Game.legalPlays(state);
            let play = plays[Math.floor(Math.random() * plays.length)];
            state = Game.nextState(state, play);
            winner = winnerFinder(state.board);
        }
